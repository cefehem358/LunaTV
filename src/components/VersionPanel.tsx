    ...
    >
      ...
      >
        ...
        <div className="p-6 border-b border-zinc-900 flex justify-between items-center bg-zinc-900/20">
          ...
          <button onClick={handleClose} className="text-zinc-500 hover:text-white transition text-lg px-2 cursor-pointer">✕</button>
        </div>
          ...
          <div className="space-y-3">
            {CHANGELOGS.map((log, index) => (
              <div key={log.version} className={`p-4 border rounded-2xl transition duration-300 ${index === 0 ? 'bg-zinc-900/50 border-[#e50914]/30' : 'bg-zinc-900/10 border-zinc-900'}`}>
                ...
                <p className="text-xs text-zinc-400 mt-2 leading-relaxed pl-2 border-l border-zinc-800 whitespace-pre-line">{log.content}</p>
              </div>
            ))}
          </div>
        </div>
