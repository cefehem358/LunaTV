      ...
      <div className="pl-64">
        ...
        <header
          className={`sticky top-0 z-40 h-20 flex items-center justify-between px-8 transition-all duration-300 ${
            isScrolled ? 'bg-[#040404]/60 backdrop-blur-xl border-b border-white/5' : 'bg-transparent'
          }`}
        >
          ...
          {activeNav === 'home' ? (
            <>
              ...
              {playRecords.length > 0 && (
                <section className="mb-8">
                  ...
                  <div className="relative group">
                    <button
                      onClick={() => scrollRow(continueRef as React.RefObject<HTMLDivElement>, 'left')}
                    ...
                    <button
                      onClick={() => scrollRow(continueRef as React.RefObject<HTMLDivElement>, 'right')}
                    ...
                    >
                      ...
                      {playRecords.map((record) => {
                        ...
                        return (
                          <button
                            key={record.key}
                            onClick={() => router.push(`/play?id=${id}&source=${source}`)}
                          ...
                          >
                            ...
                            <div className="absolute inset-x-0 bottom-0 p-3">
                              <p className="text-white text-sm font-medium line-clamp-1">{record.title}</p>
                ...
                <section className="relative h-[480px] rounded-3xl overflow-hidden mb-10">
                  ...
                  <div className="relative h-full flex flex-col justify-end p-10">
                    ...
                    <div className="flex gap-4">
                      <button
                        onClick={() => router.push(`/play?id=${featuredItem.id}&source=douban`)}
                      ...
                      <button
                        onClick={() => router.push(`/info?id=${featuredItem.id}&source=douban`)}
              ...
              <section className="mb-10">
                <SectionTitle title="最新上架" icon={<Clapperboard className="w-5 h-5 text-[#e50914]" />} viewAllHref="/douban?type=anime" />
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-7 gap-4">
                  {[...hotMovies, ...hotTvShows].slice(0, 14).map((item, idx) => (
              ...
              {/* 新番放送（今日） */}
              <NetflixBangumiRow bangumiData={bangumiData} scrollRow={scrollRow} />
...
function NetflixSectionRow({
  ...
  items: DoubanItem[];
  viewAllHref?: string;
  scrollRow: (ref: React.RefObject<HTMLDivElement>, dir: 'left' | 'right') => void;
}) {
  ...
  return (
    <section className="mb-10">
      ...
      <div className="relative group">
        <button
          onClick={() => scrollRow(scrollRef as React.RefObject<HTMLDivElement>, 'left')}
          className="absolute left-0 top-1/2 -translate-y-1/2 z-10 w-10 h-10 bg-black/60 backdrop-blur-sm rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hover:bg-black/80"
        >
          <ChevronLeft className="w-5 h-5 text-white" />
        </button>
        <button
          onClick={() => scrollRow(scrollRef as React.RefObject<HTMLDivElement>, 'right')}
          className="absolute right-0 top-1/2 -translate-y-1/2 z-10 w-10 h-10 bg-black/60 backdrop-blur-sm rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hover:bg-black/80"
        >
          <ChevronRight className="w-5 h-5 text-white" />
        </button>
...
function NetflixBangumiRow({
  ...
  const todayAnimes = bangumiData.find((d) => d.weekday.en === todayKey)?.items || [];
...
function FavoritesView() {
  ...
  const updateFavoriteItems = useCallback(
    async (allFavorites: Record<string, unknown>) => {
      ...
      const sorted = Object.entries(allFavorites)
        .sort(([, a], [, b]) => (b as { save_time: number }).save_time - (a as { save_time: number }).save_time)
        .map(([key, fav]) => {
  ...
  useEffect(() => {
    ...
    load();
    const unsub = subscribeToDataUpdates('favoritesUpdated', updateFavoriteItems);
    return unsub;
  }, [updateFavoriteItems]);
...
export function NetflixHomePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [moviesData, tvShowsData, varietyData, bangumi, records] = await Promise.all([
