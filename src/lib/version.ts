...
- 版本號提升至 1.6.1
  ...
  },
  {
    ...
    date: '2026-05-22',
    content: `
- 圖片代理開放所有外部域名（保留 SSRF 私有 IP 防護），修復卡牌圖片無法載入問題
- 播放源優選測速全並行化，備用關鍵字搜尋也改為並行，大幅縮短片源選擇等待時間
- 播放頁快取加入 1 小時 TTL 逾期檢查，防止過期 M3U8 連結導致影片載入失敗
- 播放頁支援讀取 URL episode 參數，無觀看記錄時也能跳到正確集數
- 減少載入完成後的人為延遲 (1000ms → 300ms)
- 搜尋變體順序修正：原始查詢優先於清理後查詢，精確搜尋不再被通用版覆蓋
- 搜尋 API 收集多個變體結果（最多 2 個），確保精確匹配和備用結果都納入
- 放寬模糊比對 LCS 門檻：短標題 (≤6字) 使用 LCS≥3 搭配 60% 覆蓋率，減少漏配
- 版本號提升至 1.6.0
    `.trim(),
  },
  {
